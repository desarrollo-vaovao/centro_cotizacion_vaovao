...
**Note:** this file has a `.js` extension but the project's build only parses JSX inside `.jsx` files — use `React.createElement` for the `wrapper` helper below (not `<QueryClientProvider>...</QueryClientProvider>` JSX syntax), exactly as shown. This is the same fix Task 6 already applied to its own `.js`-extension test files with a JSX wrapper.
...
**Note:** this file has a `.js` extension but the project's build only parses JSX inside `.jsx` files — use `React.createElement` for the `wrapper` helper below, exactly as shown (same fix Task 6 and Task 8 already applied to their own `.js`-extension test files with a JSX wrapper).
