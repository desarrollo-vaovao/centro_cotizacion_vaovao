...
6. El servidor ya configura `app.set('trust proxy', 1)` (ver `src/app.js`). Esto es necesario porque Railway termina TLS un salto antes del proceso y reenvía HTTP plano: sin `trust proxy`, Express nunca ve `req.secure` como verdadero, así que `express-session` con `cookie.secure = true` no envía la cookie de sesión (el login "funciona" pero el navegador nunca la recibe, y todo pedido posterior da 401). También es necesario para que `express-rate-limit` identifique correctamente la IP de cada cliente en vez de agrupar a todos bajo la IP del proxy de Railway.
